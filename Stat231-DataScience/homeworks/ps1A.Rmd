...
g <- ggplot(data = CIACountries, aes(y = gdp, x = educ)) + geom_text(aes(label = country, color = net_users), size = 3)
...
```{r}
g <- g + xlab("% of GDP spent on education") +
...
> ANSWER: The x axis, medical procedure, is no longer sorted by average cost.
...
> ANSWER: It adjusted the labels for individual procedures - I'm guessing that's what the numbers represent - to be legible by rotating them 90 degrees to make even, appropriate space for each one along the x-axis.
...
> ANSWER: What does 'stat = "indentity"' do in the geom_bar function? Why do all those warning about removing rows pop up when you use individual functions, and what exactly are they doing when removing data? Is make_babynames_dist() seriously a built-in function, or is it defined somewhere else? Will we learn to make our own functions in R (How?)?
