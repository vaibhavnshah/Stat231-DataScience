...
```{r}
coverage <- read_csv("/Users/Vaibhav7/git/Stat231-DataScience/Stat231-DataScience/data/healthcare_coverage.txt")
spending <- read_csv("/Users/Vaibhav7/git/Stat231-DataScience/Stat231-DataScience/data/healthcare_spending.txt")
```
...
> RESPONSE: Spending covers 1991 - 2014. Spending covers 2013-2015.

```{r}
spending <- spending %>% select(contains("2013") | contains("2014") | contains("Location"))
coverage <- coverage %>% select(contains("2013") | contains("2014") | contains("Location"))
spending 
coverage
```
...
```{r}
coverage <- coverage %>% filter(Location != "United States") %>% filter(Location != "District of Columbia")
spending <- spending %>% filter(Location != "United States") %>% filter(Location != "District of Columbia")
```
...
> RESPONSE: The Employer column from coverage and the total spending column from spending will be used. We'll have to mutate the data to have logarithmic values of both. Each row will be a different state.

What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: Mutate some logarithmic values in. Combine desired columns from both datasets into one dataset. 
...
```{r}
plotdata <- merge(coverage, spending, by = c("2013_Employer", "2014_Employer", "2013_Total Health Spending", "2014_Total Health Spending"))
```
